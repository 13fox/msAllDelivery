msAllDelivery.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper msalldelivery-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('msalldelivery-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('msalldelivery-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('msalldelivery-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('msalldelivery-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    msAllDelivery.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(msAllDelivery.grid.Default, MODx.grid.Grid, {
    ...
    getColumns: function () {
        return [{
            ...
            header: _('msalldelivery_actions'),
            dataIndex: 'actions',
    ...
    getSearchField: function (width) {
        return {
            xtype: 'msalldelivery-field-search',
            width: width || 250,
            listeners: {
                search: {
                    fn: function (field) {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = msAllDelivery.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('msalldelivery-grid-default', msAllDelivery.grid.Default);